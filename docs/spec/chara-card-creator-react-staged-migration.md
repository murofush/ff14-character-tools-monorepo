...
- 2026-02-17 時点で `/edit-chara-card` は以下を追加移植済み。
  ...
  - ジョブアイコン表示を候補URLフォールバック方式で移植（旧 `getJobSubImagePath` ソース未確認のため推定互換）
  - 旧Konva準拠の文字組み調整（実績タイトル/日付の重なり回避、説明文省略、報酬行配置）を反映
  - 旧Canvas責務の `Since` 表示とLodestone URL/copyright フッタ表示をプレビュー・PNG出力へ反映
  - `Since` 表示を旧Vue互換で拡張し、最古実績日に応じて世代/パッチ補足（例: `Legacy:1.X` / `漆黒:5.0`）を付与
  - `/edit-chara-card` 設定UIで、緩和前取得実績が存在する場合のみ「強調表示無効化」チェックを表示する旧条件を反映
  - `/edit-chara-card` 設定UIの「全体（JP）」フォント候補をJPフォント一覧のみに制限（旧設定互換）
  - 旧 `OUTPUT_SNACKBAR` 相当の全画面共通通知レイヤーを追加し、Home/Select/Editの主要失敗・成功通知を共通化
  - 共通ヘッダ導線を旧責務同等ラベル（キャラクター選択/アチーブメント選択/名刺デザイン編集）へ更新し、未取得時は選択/編集導線を無効化
  - 取得済みキャラクター情報メニュー（名前/プロフィール要約/Lodestone導線）をヘッダへ追加
  - フッタへ問い合わせフォーム導線と `/about` 導線を追加
